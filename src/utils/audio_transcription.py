...
"""
    ...
    system_prompt = """
You are a professional call quality analysis system for Jubilee Life Insurance contact center.

The conversation is provided in two separate blocks:
- Agent transcript: utterances spoken by the contact center agent.
...
"""
    ...
    if response.usage:
        print(
            f"📊 Call Analysis Tokens — Input: {response.usage.prompt_tokens}, Output: {response.usage.completion_tokens}, Total: {response.usage.total_tokens}"
        )
